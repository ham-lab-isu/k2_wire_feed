...
#include "k2_wire_feed/action/wire_feed.hpp"
#include "Axis.h"
...
using WireFeed = k2_wire_feed::action::WireFeed;
/******* Wire Feed Action Callbacks ************************************** */
// Action Handle Goal Request + Response
rclcpp_action::GoalResponse handle_goal_callback(
    const rclcpp_action::GoalUUID & uuid,
    std::shared_ptr<const WireFeed::Goal> goal)
{
    RCLCPP_INFO(rclcpp::get_logger("Teknic"), "Received goal request");
    (void)uuid;  // Avoid unused variable warning
    (void)goal;
    return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
}

// Action Handle Cancel Request + Response
rclcpp_action::CancelResponse handle_cancel_callback(
    const std::shared_ptr<rclcpp_action::ServerGoalHandle<WireFeed>> goal_handle)
{
    RCLCPP_INFO(rclcpp::get_logger("Teknic"), "Received request to cancel goal");
    (void)goal_handle;
    return rclcpp_action::CancelResponse::ACCEPT;
}

// Action Handle Accepted Goal
void handle_accepted_callback(
    const std::shared_ptr<rclcpp_action::ServerGoalHandle<WireFeed>> goal_handle)
{
    RCLCPP_INFO(rclcpp::get_logger("Teknic"), "Executing goal");
    (void)goal_handle;
    // You can start a new thread here to execute the goal
}
/*************************************************************************** */
...
int main(int argc, char* argv[])
{
	...
	auto action_server = rclcpp_action::create_server<WireFeed>(
    	node,
    	"wire_feed_action_server",
    	std::bind(&handle_goal_callback, std::placeholders::_1, std::placeholders::_2),
    	std::bind(&handle_cancel_callback, std::placeholders::_1),
    	std::bind(&handle_accepted_callback, std::placeholders::_1)
);
	...
	RCLCPP_INFO(rclcpp::get_logger("Teknic"), "----------- RUNNING TEKNIC NODE -----------");
	...
	{ 
		...
		RCLCPP_INFO(rclcpp::get_logger("Teknic"), "----------- Teknic found %d SC Hubs -------------", comHubPorts.size());
			...
			for (unsigned iNode = 0; iNode < myPort.NodeCount(); iNode++){
				...
				if (nodeTypesGood) {
					...
					/*
						...
						// Set the move distance in revolutions
						float64 revs_requested = distance/(3.14159*roller_diameter)
						listOfAxes.at(iNode)->SetMoveRevs(revs_requested);
