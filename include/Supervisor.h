...
public:
	...
	// Quit the motion on a specific axis
	void AxisQuit(int requested_axis){
		printf("  Supervisor quitting specific axis\n");
		m_quitting = true;
		// check that the axis exists
		if (requested_axis>=0 && requested_axis<m_axes.size()){
			m_axes.at(requested_axis)->Quit();
		} else{
			printf("Axis not found; try a different axis\n");
		}
	}
